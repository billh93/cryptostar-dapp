...
it('can Create a Star', async() => {
    ...
    assert.equal(await instance.tokenIdToStarInfo.call(tokenId), 'Test Star')
});
...
it('can add the star name and star symbol properly', async() => {
    let tokenId = 6
    let instance = await StarNotary.deployed()
    await instance.createStar('Bills First Token', tokenId, {from: accounts[0]})
    let name = await instance.name.call()
    let symbol = await instance.symbol.call()
    assert.equal(name + ' ' + symbol, "Bills First Token BFT")
});
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Test Star!', tokenId1, {from: user1});
    await instance.createStar('Test Star!', tokenId2, {from: user2});
    // 2. Call the exchangeStars functions implemented in the Smart Contract
    await instance.exchangeStars( tokenId1 , tokenId2 , {from: user1} );
    const owner1 = await instance.ownerOf.call(tokenId1);
    const owner2 = await instance.ownerOf.call(tokenId2)
    // 3. Verify that the owners changed
    assert.equal(owner1 + owner2, user2 + user1)
});
